% Wiener-residual centroid classifier on the full RGB residual.
% Same pipeline as the gray and green versions but keeps all three
% channels, with normalisation and correlation taken jointly across
% the H x W x 3 volume.
...
testRoot  = "data/test/";
outFile   = "results/Wiener_Centroids_Color.mat";

epsVal = 1e-12;

% ---- training: build one centroid per class ----
fprintf("Computing Wiener centroids (color)\n");
...
for c = 1:numClasses
    ...
    for k = 1:numFiles
        ...
        Z = double(data.residual);          % H x W x 3
        Z = Z - mean(Z, 'all');             % zero-mean over all channels jointly
...
fprintf("\nTesting Wiener correlation (color)\n");
...
fprintf("\nColor Wiener accuracy: %.2f%%\n", acc * 100);

figure;
confusionchart(allTrueIdx, allPredIdx);
title(sprintf("Color Wiener Accuracy: %.2f%%", acc * 100));
